...
path = '/home/maddy/Code/data_analysis/Flip_Detect/fiscella_2020/Flip_Detect_Data/'
...
#percent correct arrays   
    ...
    disagree_upright[j][0] = dp_disagree[0, 1] #upright target
    disagree_upright[j][1] = dp_disagree[0, 0] #upright masker   
    
    disagree_inverted[j][0] = dp_disagree[1, 1] # flipped target
    disagree_inverted[j][1] = dp_disagree[1, 0] #flipped masker
    
    disagree_tvm[j][0] = (dp_disagree[0, 1] - dp_disagree[0, 0]) #upright target - upright masker
    disagree_tvm[j][1] = (dp_disagree[1, 1] - dp_disagree[1, 0]) #flipped target - flipped masker
    
    disagree_up_vs_inv[j][0] = (dp_disagree[0, 1] - dp_disagree[0, 0])-(dp_disagree[1, 1] - dp_disagree[1, 0]) #upright - inverted
    

    pc_disagree_conditions[0][j]= (dp_disagree[0, 1] - dp_disagree[0, 0]) #upright target - upright masker
    pc_disagree_conditions[1][j]=(dp_disagree[1, 1] - dp_disagree[1, 0]) #inverted target - inverted masker
    pc_disagree_conditions[2][j]=(dp_disagree[0, 1] - dp_disagree[0, 0]) - (dp_disagree[1, 1] - dp_disagree[1, 0]) #upright - inverted
...
performance = [disagree_upright.mean(0)[0]-0,disagree_upright.mean(0)[1]-0]
performanceSE = [disagree_upright.std(0)[0] / np.sqrt(len(subjects)),disagree_upright.std(0)[1] / np.sqrt(len(subjects))]
plt.bar((0,1), performance, width=barw, bottom=0, align='center', alpha=1, edgecolor=color_up, facecolor = 'w', zorder=-100)
plt.errorbar(x=(0,1), y=np.array(performance)+0, c=color_up, fmt=marker_up, ms=ms, mec=color_up, mfc='w', capsize=3, yerr=performanceSE) 
...
performance = [disagree_inverted.mean(0)[0]-0,disagree_inverted.mean(0)[1]-0]
performanceSE = [disagree_inverted.std(0)[0] / np.sqrt(len(subjects)),disagree_inverted.std(0)[1] / np.sqrt(len(subjects))]
plt.bar((0, 1), performance, width=barw, bottom=0, align='center', alpha=1, edgecolor=color_down, facecolor = 'w', zorder=-100)
plt.errorbar(x=(0,1), y=np.array(performance)+0, c=color_down, fmt=marker_down, ms=ms, mec=color_down, mfc='w', capsize=3, yerr=performanceSE)
plt.plot([-1, 2], [0, 0], 'k--', lw=.5, zorder=-200)
plt.xlim([-.25, 1.25])
plt.xticks([0, 1], [0, 1])
plt.ylim([-.5, 4.5])
...
inverted_dprimes = dprime_diss[:, 2:, np.newaxis]
total_dprimes = np.concatenate((upright_dprimes, inverted_dprimes), 2)

subject, match, rotation = np.meshgrid(
        np.arange(23), ['T', 'M'], ['Upright', 'Invert'], indexing='ij')

data_dict = dict(subj=subject.ravel(),
                 match=match.ravel(),
                 angle=rotation.ravel(),
                 dpc=total_dprimes.ravel())
data = DataFrame(data_dict)

formula = 'dpc ~ C(angle) + C(match)+ C(angle):C(match)'
